...
function InfoRow({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 border-b border-slate-100/60 py-2.5 last:border-0 dark:border-slate-800/60 sm:items-center">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800">
        <Icon className="h-4 w-4 text-brand-600 dark:text-brand-300" />
      </div>
      <span className="shrink-0 pt-1 text-sm text-slate-500 dark:text-slate-400 sm:pt-0">{label}</span>
      <span className="ml-auto min-w-0 break-words text-right text-sm font-semibold text-slate-900 dark:text-white">
        {value}
      </span>
    </div>
  );
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="max-w-full truncate px-1 text-sm text-slate-500 dark:text-slate-400" title={user.email}>
                {user.email}
              </p>
              <span
